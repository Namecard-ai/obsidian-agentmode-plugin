		...
		this.plugin.tokenRefreshTimer = this.plugin.registerInterval(
			window.setInterval(() => {
				if (this.plugin.settings.isLoggedIn && this.isTokenExpiringSoon()) {
					this.autoRefreshToken().catch((error: unknown) => {
...
export default class AgentPlugin extends Plugin {
	...
	async streamAgentChat(
		...
		// eslint-disable-next-line no-constant-condition -- Intentional infinite loop with internal break conditions
		while (true) {
...
- Consider visual hierarchy and flow direction
	...
	// eslint-disable-next-line @typescript-eslint/no-unused-vars -- Used in AgentChatView.tsx via constructor property access (TypeScript cannot detect this usage)
